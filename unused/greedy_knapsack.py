...
def get_best_player(gks):
    ...
    best_player = df.nlargest(10, ['Points']).values.tolist()
...
def local(players, W):
    ...
    for index, player in enumerate(lineup):
        ...
        if player[1] == 'MID':
            pom_lineup[index] = best_mid.pop()
        elif player[1] == 'GK':
            pom_lineup[index] = best_gk.pop()
        elif player[1] == 'DEF':
            pom_lineup[index] = best_def.pop()
        elif player[1] == 'FW':
            pom_lineup[index] = best_fw.pop()
    ...
    novi_trosak= [x[5] for x in novi_lineup]
    print (sum(dobit))
    print (sum(novi_trosak))
